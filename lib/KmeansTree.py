"""
KmeansTree.py. TODO

实现 balanced k-means 聚类树，用来把 item_num 个物品编码到一棵 k 叉树的叶子上。
算法是先大组聚类、再递归细分的模式。
"""

import time
import collections
import multiprocessing as mp
import math
from joblib import Parallel, delayed
import queue as Que
import torch


def pairwise_distance_function(data1, data2):
    """计算两组向量之间的欧氏距离矩阵
    data1 是 [m, dim] 的 m 个物品特征向量，
    data2 是 [k, dim] 的 k 个聚类中心（centroids）
    输出 [m, k] 的矩阵，[i, j] 表示物品 i 到中心 j 的欧几里得距离平方。
    """
    # data1 [m,dim]
    # data2 [k,dim]
    return ((data1.unsqueeze(dim=1) - data2) ** 2.0).sum(-1)  # [m,k]


def initialize(X, num_clusters):
    """	随机初始化 num_clusters 个聚类中心
    X 是 [total_items, dim]，
    先把 X 随机排列，再 reshape 成 [num_clusters, cluster_size, dim]，然后每个聚类片的平均值作为该簇的初始聚类中心。
    """
    indices = torch.randperm(X.shape[0], device=X.device)
    
    # [num_clusters, dim]
    return X[indices].view(num_clusters, -1, X.shape[-1]).mean(dim=1)


def kmeans_equal(
    X,
    num_clusters=2,
    cluster_size=10,
    max_iters=100,
    initial_state=None,
    update_centers=True,
    tol=1e-6,
):
    """平衡 k-means —— 每簇大小严格相等
    
    这是传统 k-means 的变体。不能自由选择最近的聚类中心（否则会导致聚类大小不均），而是通过迭代分配，确保每个簇的样本数恰好等于 cluster_size。
    
    算法步骤：

    初始化中心（第 39-41 行）： 如果没提供初始中心，就用 initialize 函数从 X 中采样。
    主循环（第 47-87 行）：
    计算每个点到所有聚类中心的距离并排序（第 50 行） → choices 是一个 [n, k] 的索引矩阵。
    对于每个簇（torch.randperm 随机顺序，第 55 行）：
    从未分配的样本中选出那些认为“该簇最匹配”的样本（第 56-58 行）。
    选择最近的 cluster_size 个样本作为该簇的成员（第 61-63 行）。
    从 left_index（未分配样本集合）中移除这些已分配的样本（第 67 行），并记录分组（第 66 行）。
    如果 update_centers，用选中的样本更新中心（第 70-75 行）。
    计算中心的前后变化 center_shift，小于阈值则提前结束（第 76-84 行）。
    核心收益是保证每个簇的大小精确等于 cluster_size，满足了后续构建 k 叉树时每个节点要均匀划分为 k 组的要求。
    """

    assert X.shape[0] == num_clusters * cluster_size, (
        "data point size should be the product of num_clusters and cluster_size"
    )
    if initial_state is None:
        # randomly group vectors to clusters (forgy initialization)
        initial_state = initialize(X, num_clusters)  ##[num_clusters,dim]
    iteration = 0

    final_choice = torch.full((X.shape[0],), -1, dtype=torch.int64, device=X.device)
    left_index = torch.full((X.shape[0],), True, dtype=torch.bool, device=X.device)
    all_ins_ids = torch.arange(X.shape[0], device=X.device)
    while True:
        # choices is [num_sample,num_cluster],remark the cluster rank
        # start_t = time.time()
        choices = torch.argsort(pairwise_distance_function(X, initial_state), dim=-1)
        # print(time.time()-start_t)

        initial_state_pre = initial_state.clone()
        left_index[:] = True
        for index in torch.randperm(num_clusters):
            cluster_positions = torch.argmax(
                (choices[left_index] == index).to(torch.long), dim=-1
            )  # cluster_positions is [left_num_sample]

            # choose the most colse cluster_size samples to cluster index,selected_ind is [cluster_size]
            selected_ind = all_ins_ids[left_index].gather(
                dim=0, index=torch.argsort(cluster_positions, dim=-1)[:cluster_size]
            )
            # print(selected_ind)

            final_choice.scatter_(0, selected_ind, value=index)
            left_index.scatter_(0, selected_ind, value=False)
            # update cluster center

            if update_centers:  # initial_state is [num_clusters,dim]
                initial_state[index] = torch.gather(
                    X,
                    0,
                    index=selected_ind.view(-1, 1).expand(cluster_size, X.shape[1]),
                ).mean(dim=0)
        center_shift = torch.sqrt(
            torch.sum((initial_state - initial_state_pre) ** 2, dim=1)
        ).sum()

        # increment iteration
        iteration = iteration + 1

        if center_shift**2 < tol:
            break
        if iteration >= max_iters:
            break

    return final_choice, initial_state


class ConstructKmeansTree:
    def __init__(self, parall):
        self.timeout = 5
        self.parall = parall


    def train(self, data=None, k=4, max_iters=100, feature_ratio=0.8):
        """ 树的构建入口，队列驱动，大组 _kmeans、小组 _minbatch
        :param data: [num_all_leaf_code, emb_size]
        """
        assert data is not None, "provide data please"
        self.k = k
        self.max_iters = max_iters
        self.feature_ratio = feature_ratio
        # id and data are tensors

        # [num_all_leaf_code, emb_size]
        self.data = data
        
        # [num_all_leaf_code, ]
        self.ids = torch.arange(data.shape[0], dtype=torch.int64)

        queue = Que.Queue()
        mini_batch_queue = Que.Queue()
        print("get into cluster training process")

        while True:
            try:
                print("start")
                queue.put((0, self.ids), timeout=self.timeout)  # parent code and index
                break
            except:
                print("put item into queue error!!")
                pass

        assert queue.qsize() == 1
        print("start to cluster")

        while queue.qsize() > 0:
            pcode, index = queue.get()
            if len(index) <= 1024:
                # self._minbatch(pcode, index, code)
                while True:
                    try:
                        mini_batch_queue.put((pcode, index), timeout=self.timeout)
                        break
                    except:
                        print("1024 mini batch error")
                        pass
            else:
                tstart = time.time()
                result_matrix = self._kmeans(index)
                print("Train iteration done, pcode:{}, data size: {}, elapsed time: {}".format(pcode, len(index), time.time() - tstart))
                self.timeout = int(0.4 * self.timeout + 0.6 * (time.time() - tstart))
                if self.timeout < 5:
                    self.timeout = 5

                for c in range(self.k):
                    while True:
                        try:
                            queue.put((self.k * pcode + c + 1, result_matrix[c]), timeout=self.timeout,)  # cluster is from root to leaf node
                            break
                        except:
                            print("err")
                            pass

        print("start to process mini-batch parallel.....................................")
        tstart = time.time()
        qcodes, indice = [], []
        while mini_batch_queue.qsize() > 0:
            pcode, index = mini_batch_queue.get()
            qcodes.append(pcode)
            indice.append(index)
        make_job = delayed(self._minbatch)
        re = Parallel(n_jobs=self.parall)(
            make_job(pcode, index) for pcode, index in zip(qcodes, indice)
        )
        # re = [self._minbatch(pcode,index) for pcode,index in zip(qcodes, indice)]
        id_code_list = []  # (item_id,code),the code is the code of leaf node
        for r in re:
            id_code_list.extend(r)
        id_code_list.sort(key=lambda x: x[0])
        ids = torch.LongTensor([id for (id, _) in id_code_list])
        codes = torch.LongTensor([code for (_, code) in id_code_list])
        print("cluster all the nodes, cost {} s".format(time.time() - tstart))
        assert (codes <= 0).sum() <= 0
        assert queue.qsize() == 0
        assert len(ids) == len(data)
        # builder.build(ids, codes, stat=self.stat, kv_file=self.kv_file)
        return (ids, codes,)


    def _minbatch(self, pcode, index):
        """对小批量递归聚类直到每组 k 个或以下
        
        对每组 ≤1024 的样本递归聚类，直到每簇大小恰好等于 k 时记为叶子节点。叶子节点格式为 (index[i].item(), self.k * pcode + 1 + i) —— 这个 leaf_code 采用类似堆的编码：父节点为 pcode，子节点为 k*pcode + 1 + i。
        """
        # pocde is paretn code,index is the assinged items' id, code is np.zeros(len(ids)), used tor recoder whetert the item processed as a leaf
        dq = collections.deque()
        dq.append((pcode, index))
        batch_size = len(index)
        id_code_list = []  # (item_id,node_code)
        tstart = time.time()
        while dq:
            pcode, index = (
                dq.popleft()
            )  # pop the tuple which is added into the deque early
            if len(index) == self.k:
                for i in range(self.k):
                    id_code_list.append(
                        (index[i].item(), self.k * pcode + 1 + i)
                    )  # (in,code) pair
            elif len(index) > self.k:
                result_matrix = self._kmeans(index)  # divide the index into two nodes
                if result_matrix.shape[1] >= self.k:
                    for c in range(self.k):
                        dq.append((self.k * pcode + c + 1, result_matrix[c]))
                else:
                    assert False, "wrong size of index"
            else:
                assert False, "wrong size of index"
        print(
            "Minbatch, batch size: {}, elapsed: {}".format(
                batch_size, time.time() - tstart
            )
        )
        return id_code_list

    def _kmeans(self, index):
        """对一组物品做一次 balanced k-means 聚类，拆成 k 组
        调用 kmeans_equal 将一组样本递归划分为 self.k 个大小相等的子簇。使用 feature_ratio 随机选取部分特征压缩计算量。输入是一组物品的原始索引，直接在索引映射基础上做 k-means 聚类，返回 (self.k, cluster_size) 的索引矩阵。
        """
        # index is tensor
        # make sure that index.length is greater or equal to k, data is tensor matrix
        # data is tensor
        assert len(index) % self.k == 0
        cluster_size = len(index) // self.k
        selected_feature = torch.randperm(self.data.shape[1])[: math.ceil(self.feature_ratio * self.data.shape[1])]
        choices, _ = kmeans_equal(
            self.data[:, selected_feature][index],
            cluster_size=cluster_size,
            num_clusters=self.k,
            max_iters=self.max_iters,
        )
        result_index = torch.full(
            (self.k, cluster_size), -1, dtype=torch.int64, device=index.device
        )
        for c in range(self.k):
            result_index[c] = index[choices == c]
        return result_index
